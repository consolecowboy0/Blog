...
export async function POST({ request, clientAddress }) {
  ...
  if (action === 'send') {
    ...
    if (!rlIp.ok) return new Response(JSON.stringify({ error: 'Rate limited' }), {
      status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json', 'Retry-After': String(rlIp.retryAfter) },
    });
    ...
    if (!rlVis.ok) return new Response(JSON.stringify({ error: 'Rate limited' }), {
      status: 429, headers: { ...corsHeaders, 'Content-Type': 'application/json', 'Retry-After': String(rlVis.retryAfter) },
    });
