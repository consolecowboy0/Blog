...
export async function POST({ request, clientAddress }) {
  ...
  // Same password as the analytics dashboard.
  if (!requireAuth(request, 'analytics')) {
  ...
  const rl = checkRate(`story-chat:${ip}`, 60, 60 * 1000);
  if (!rl.ok) {
  ...
  const maxTok = Math.min(1500, Math.max(100, parseInt(max_tokens, 10) || 600));
