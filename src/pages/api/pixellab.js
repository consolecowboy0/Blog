...
export async function POST({ request, clientAddress }) {
  ...
  if (!apiKey) {
    return new Response(JSON.stringify({ error: "Image service not available" }), {
      status: 500,
      headers: corsHeaders,
    });
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return new Response(
        JSON.stringify({ error: "Image generation service error" }),
    ...
    return new Response(
      JSON.stringify({ error: "Failed to call image generation service" }),
